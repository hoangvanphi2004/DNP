...
from transformerMMCV import ScaleNorm
...
class Scale(nn.Module):
    ...
    def __init__(self, dim, init_value=1., trainable=True):
        ...
        self.scale = nn.Parameter(
            init_value * torch.ones(dim, device = "cuda"), requires_grad=trainable)
    ...
    def __init__(self,
        ...
        if use_rel_bias:
            if attn_type == 'self-attn':
                self.w = nn.Parameter(
                    torch.rand([2 * num_token - 1], dtype=torch.float, device = "cuda"))
            else:
